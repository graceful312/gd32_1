...
- **FatFs** (`FatFs/FatFs.c`)：FAT 文件系统 R0.09 封装，底层 `diskio.c` 对接 SPI Flash。512B 逻辑扇区，1024 个扇区（512KB）。首次使用自动格式化并挂载。API 参考 `ff.h`。配置 `ffconf.h`：代码页 936（GBK 中文），已开启 `_USE_MKFS`。Keil 需添加 `FatFs/` 到 Include Paths。
